...
export function destroy_empty_ (
  ...
  while (($.copy(i)).lt($.copy(map.num_buckets))) {
    {
      Std.Vector.destroy_empty_(Table_with_length.remove_(map.buckets, $.copy(i), $c, [AtomicTypeTag.U64, new VectorTag(new SimpleStructTag(Entry, [$p[0], $p[1]]))]), $c, [new SimpleStructTag(Entry, [$p[0], $p[1]])]);
      i = ($.copy(i)).add(u64("1"));
    }
...
export function new___ (
  ...
  Table_with_length.add_(buckets, u64("0"), Std.Vector.empty_($c, [new SimpleStructTag(Entry, [$p[0], $p[1]])]), $c, [AtomicTypeTag.U64, new VectorTag(new SimpleStructTag(Entry, [$p[0], $p[1]]))]);
