...
export function add_element_ (
  ...
  let temp$1, temp$2;
  [temp$1, temp$2] = [v, x];
  if (!Std.Vector.contains_(temp$1, temp$2, $c, [$p[0]])) {
    Std.Vector.push_back_(v, x, $c, [$p[0]]);
  }
  else{
  }
  return;
...
export function create_ (
  ...
  if (!!$c.exists(new SimpleStructTag(CapState, [$p[0]]), $.copy(addr))) {
    throw $.abortCode(Std.Error.already_exists_($.copy(ECAPABILITY_ALREADY_EXISTS), $c));
  }
  $c.move_to(new SimpleStructTag(CapState, [$p[0]]), owner, new CapState({ delegates: Std.Vector.empty_($c, [AtomicTypeTag.Address]) }, new SimpleStructTag(CapState, [$p[0]])));
  return;
}
...
export function delegate_ (
  ...
  let addr;
  addr = Std.Signer.address_of_(to, $c);
...
export function remove_element_ (
  ...
  [temp$1, temp$2] = [v, x];
  [found, index] = Std.Vector.index_of_(temp$1, temp$2, $c, [$p[0]]);
  if (found) {
    Std.Vector.remove_(v, $.copy(index), $c, [$p[0]]);
  }
  else{
  }
  return;
...
export function validate_acquire_ (
  ...
  addr = Std.Signer.address_of_(requester, $c);
  if ($c.exists(new SimpleStructTag(CapDelegateState, [$p[0]]), $.copy(addr))) {
    ...
    if (!$c.exists(new SimpleStructTag(CapState, [$p[0]]), $.copy(root_addr))) {
      throw $.abortCode(Std.Error.invalid_state_($.copy(EDELEGATE), $c));
    }
    if (!Std.Vector.contains_($c.borrow_global<CapState>(new SimpleStructTag(CapState, [$p[0]]), $.copy(root_addr)).delegates, addr, $c, [AtomicTypeTag.Address])) {
      throw $.abortCode(Std.Error.invalid_state_($.copy(EDELEGATE), $c));
    }
    ...
    if (!$c.exists(new SimpleStructTag(CapState, [$p[0]]), $.copy(addr))) {
      throw $.abortCode(Std.Error.not_found_($.copy(ECAPABILITY_NOT_FOUND), $c));
    }
    temp$1 = $.copy(addr);
  }
  return temp$1;
