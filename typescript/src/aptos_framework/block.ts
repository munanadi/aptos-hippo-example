...
export class BlockResource 
{
  ...
  update_epoch_interval_events: Aptos_std.Event.EventHandle;
  ...
  constructor(proto: any, public typeTag: TypeTag) {
    ...
    this.update_epoch_interval_events = proto['update_epoch_interval_events'] as Aptos_std.Event.EventHandle;
  }
...
export function initialize_ (
  ...
  $c.move_to(new SimpleStructTag(BlockResource), aptos_framework, new BlockResource({ height: u64("0"), epoch_interval: $.copy(epoch_interval_microsecs), new_block_events: Aptos_std.Event.new_event_handle_(aptos_framework, $c, [new SimpleStructTag(NewBlockEvent)]), update_epoch_interval_events: Aptos_std.Event.new_event_handle_(aptos_framework, $c, [new SimpleStructTag(UpdateEpochIntervalEvent)]) }, new SimpleStructTag(BlockResource)));
  return;
}
...
export function update_epoch_interval_microsecs_ (
  ...
  Aptos_std.Event.emit_event_(block_resource.update_epoch_interval_events, new UpdateEpochIntervalEvent({ old_epoch_interval: $.copy(old_epoch_interval), new_epoch_interval: $.copy(new_epoch_interval) }, new SimpleStructTag(UpdateEpochIntervalEvent)), $c, [new SimpleStructTag(UpdateEpochIntervalEvent)]);
  return;
}
