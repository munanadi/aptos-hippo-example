...
export function burn_ (
  ...
  if (Std.Option.is_some_(maybe_supply, $c, [new StructTag(new HexString("0x1"), "optional_aggregator", "OptionalAggregator", [])])) {
    supply = Std.Option.borrow_mut_(maybe_supply, $c, [new StructTag(new HexString("0x1"), "optional_aggregator", "OptionalAggregator", [])]);
...
export function initialize_internal_ (
  ...
  if (monitor_supply) {
    temp$1 = Std.Option.some_(Optional_aggregator.new___($.copy(MAX_U128), parallelizable, $c), $c, [new StructTag(new HexString("0x1"), "optional_aggregator", "OptionalAggregator", [])]);
  }
  else{
    temp$1 = Std.Option.none_($c, [new StructTag(new HexString("0x1"), "optional_aggregator", "OptionalAggregator", [])]);
  }
...
export function supply_ (
  ...
  if (Std.Option.is_some_(maybe_supply, $c, [new StructTag(new HexString("0x1"), "optional_aggregator", "OptionalAggregator", [])])) {
    supply = Std.Option.borrow_(maybe_supply, $c, [new StructTag(new HexString("0x1"), "optional_aggregator", "OptionalAggregator", [])]);
    value = Optional_aggregator.read_(supply, $c);
    temp$1 = Std.Option.some_($.copy(value), $c, [AtomicTypeTag.U128]);
  }
  else{
...
export function upgrade_supply_ (
  ...
  if (!$.copy($c.borrow_global_mut<SupplyConfig>(new SimpleStructTag(SupplyConfig), new HexString("0x1")).allow_upgrades)) {
    throw $.abortCode(Std.Error.permission_denied_($.copy(ECOIN_SUPPLY_UPGRADE_NOT_SUPPORTED), $c));
  }
