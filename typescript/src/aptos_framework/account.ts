...
export function accept_rotation_capability_ed25519_ (
  ...
  if (!Std.Option.contains_(account_resource.rotation_capability_offer.for__, addr, $c, [AtomicTypeTag.Address])) {
  ...
  Std.Option.extract_(account_resource.rotation_capability_offer.for__, $c, [AtomicTypeTag.Address]);
...
export function initialize_ (
  ...
  $c.move_to(new SimpleStructTag(ChainSpecificAccountInfo), aptos_framework, new ChainSpecificAccountInfo({ module_addr: new HexString("0x1"), module_name: [u8("97"), u8("99"), u8("99"), u8("111"), u8("117"), u8("110"), u8("116")], script_prologue_name: $.copy(script_prologue_name), module_prologue_name: $.copy(module_prologue_name), multi_agent_prologue_name: $.copy(multi_agent_prologue_name), user_epilogue_name: $.copy(user_epilogue_name) }, new SimpleStructTag(ChainSpecificAccountInfo)));
  $c.move_to(new SimpleStructTag(OriginatingAddress), aptos_framework, new OriginatingAddress({ address_map: Aptos_std.Table.new___($c, [AtomicTypeTag.Address, AtomicTypeTag.Address]) }, new SimpleStructTag(OriginatingAddress)));
  return;
}
...
export function offer_rotation_capability_ed25519_ (
  ...
  pubkey = Aptos_std.Ed25519.new_unvalidated_public_key_from_bytes_($.copy(account_public_key_bytes), $c);
  rotation_capability_sig = Aptos_std.Ed25519.new_signature_from_bytes_($.copy(rotation_capability_sig_bytes), $c);
  account_resource = $c.borrow_global_mut<Account>(new SimpleStructTag(Account), $.copy(addr));
  ...
  if (!Aptos_std.Ed25519.signature_verify_strict_t_(rotation_capability_sig, pubkey, rotation_capability_offer_proof_challenge, $c, [new SimpleStructTag(RotationCapabilityOfferProofChallenge)])) {
    throw $.abortCode($.copy(EINVALID_PROOF_OF_KNOWLEDGE));
  }
  Std.Option.fill_(account_resource.rotation_capability_offer.for__, $.copy(recipient_address), $c, [AtomicTypeTag.Address]);
  return;
}
...
export function verify_authentication_key_matches_ed25519_public_key_ (
  ...
  Std.Vector.push_back_(account_public_key_bytes, u8("0"), $c, [AtomicTypeTag.U8]);
  expected_account_auth_key = Std.Hash.sha3_256_($.copy(account_public_key_bytes), $c);
