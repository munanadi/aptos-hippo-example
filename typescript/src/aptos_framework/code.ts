...
export class PackageMetadata 
{
  ...
  upgrade_number: U64;
  source_digest: Std.String.String;
  manifest: U8[];
  ...
  constructor(proto: any, public typeTag: TypeTag) {
    ...
    this.source_digest = proto['source_digest'] as Std.String.String;
...
export function initialize_ (
  ...
  if (!$c.exists(new SimpleStructTag(PackageRegistry), $.copy(addr))) {
    ...
    Std.Vector.push_back_($c.borrow_global_mut<PackageRegistry>(new SimpleStructTag(PackageRegistry), $.copy(addr)).packages, metadata, $c, [new SimpleStructTag(PackageMetadata)]);
  }
  return;
}
...
export function publish_package_ (
  ...
  if (($.copy(index)).lt($.copy(len))) {
    $.set(Std.Vector.borrow_mut_(packages, $.copy(index), $c, [new SimpleStructTag(PackageMetadata)]), pack);
  }
  else{
    Std.Vector.push_back_(packages, pack, $c, [new SimpleStructTag(PackageMetadata)]);
  }
