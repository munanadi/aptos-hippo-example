...
export function enable_reconfiguration_ (
  ...
  $c.move_from<DisableReconfiguration>(new SimpleStructTag(DisableReconfiguration), Std.Signer.address_of_(aptos_framework, $c));
  return;
}
...
export function initialize_ (
  ...
  System_addresses.assert_aptos_framework_(aptos_framework, $c);
  if (!(Std.Guid.get_next_creation_num_(Std.Signer.address_of_(aptos_framework, $c), $c)).eq((u64("1")))) {
    throw $.abortCode(Std.Error.invalid_state_($.copy(EINVALID_GUID_FOR_EVENT), $c));
  }
  $c.move_to(new SimpleStructTag(Configuration), aptos_framework, new Configuration({ epoch: u64("0"), last_reconfiguration_time: u64("0"), events: Aptos_std.Event.new_event_handle_(aptos_framework, $c, [new SimpleStructTag(NewEpochEvent)]) }, new SimpleStructTag(Configuration)));
  return;
}
